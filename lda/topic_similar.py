...
class LdaTopicSimilar:
	...
	def f_topic_samilarity(topic_samilarity_dict1,topic_samilarity_dict2):
		...
		print (len(eachkey_len_intersection))                     
		return eachkey_len_intersection

if __name__ == '__main__':
	filelist=LdaTopicSimilar.f_file_namelist()
	topic_dict_list=[]
	...
	for topic in topic_dict_list[1:]:                                                                    #第一组实验作为初始组进行计算
		topic_samilarity_dict=LdaTopicSimilar.f_topic_samilarity(topic_dict_list[0],topic)
		for key in topic_dict_list[0]:
	...
	for key in topic_samilarity_lenth_dict:
		print (key,topic_samilarity_lenth_dict)
